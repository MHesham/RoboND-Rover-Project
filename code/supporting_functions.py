...
def create_output_images(Rover):
    ...
    if Rover.pos is not None:
        pos_x = Rover.pos[0]
        pos_y = Rover.pos[1]
        world_height = map_add.shape[0]
        world_width = map_add.shape[1]
        robot_top = int(max([0, pos_y - Rover.minimap_size]))
        robot_bottom = int(min([world_height - 1, pos_y + Rover.minimap_size]))
        robot_left = int(max([0, pos_x - Rover.minimap_size]))
        robot_right = int(min([world_width - 1, pos_x + Rover.minimap_size]))
        map_add[robot_top:robot_bottom, robot_left:robot_right] = (255, 255, 0)
    ...
    if Rover.nav_angles is not None:
        cv2.putText(map_add, "Nav#: " + str(len(Rover.nav_angles)), (0, 115),
                    cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
    cv2.putText(map_add, "Vel Stuck: " + str(Rover.is_vel_stuck), (0, 130),
                cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
    cv2.putText(map_add, "Steer Stuck: " + str(Rover.is_steer_stuck), (0, 145),
                cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
    cv2.putText(map_add, "Steer: " + str(round(Rover.steer,2)), (0, 160),
                cv2.FONT_HERSHEY_COMPLEX, 0.4, (255, 255, 255), 1)
